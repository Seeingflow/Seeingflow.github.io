...
function renderSolutions() {
  ...
  text("solutionCount", `${solutions.length} ${solutions.length === 1 ? "solution" : "solutions"} available`);
  $("solutionList").innerHTML = solutions.length ? solutions.map((solution, index) => `
    <article class="document-item ${index === 0 ? "featured-document" : ""}">
      ${solution.image ? `<img class="repository-list-image" src="${esc(solution.image)}" alt="${esc(solution.title)}">` : `<div class="document-file-icon">SOL</div>`}
      <div>
        ...
        <p>${esc(solution.text)}</p>
      </div>
      <a class="button small" href="solution-detail.html?solution=${index}">View solution</a>
    </article>
  `).join("") : `
    <article class="document-item">
      <div class="document-file-icon">SOL</div>
      <div>
        <div class="document-meta"><span>No solutions</span></div>
        <h3>Add solutions in admin</h3>
        <p>Create solution rows in the Solutions admin section.</p>
      </div>
    </article>
  `;
}
