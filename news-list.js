...
function getNewsTimestamp(item) {
  const value = getNewsDateValue(item);
  if (!value) return 0;
...
function renderNews() {
  const news = data.resources
    .map((item, index) => ({ item, index }))
    .filter(({ item }) => normalizeCategory(item.type) === "news")
    .sort((a, b) => getNewsTimestamp(b.item) - getNewsTimestamp(a.item));

  text("newsCount", `${news.length} ${news.length === 1 ? "item" : "items"} available`);
  $("newsList").innerHTML = news.length ? news.map(({ item, index }, listIndex) => `
    <article class="document-item ${listIndex === 0 ? "featured-document" : ""}">
      <div class="document-file-icon">NEWS</div>
      <div>
        <div class="document-meta">
          <span>${esc(getNewsDateValue(item) || "Undated")}</span>
          ${listIndex === 0 ? "<span>Latest</span>" : ""}
        </div>
        <h3>${esc(item.title)}</h3>
        <p>${esc(item.text)}</p>
      </div>
      <a class="button small" href="resource-detail.html?resource=${index}">Read more</a>
    </article>
  `).join("") : `
    <article class="document-item">
      <div class="document-file-icon">NEWS</div>
      <div>
        <div class="document-meta"><span>No news</span></div>
        <h3>Add news in admin</h3>
        <p>Create news rows in the Resources admin section using the News category.</p>
      </div>
    </article>
  `;
}
