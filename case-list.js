...
function renderCases() {
  ...
  text("caseCount", `${cases.length} ${cases.length === 1 ? "case" : "cases"} available`);
  $("caseList").innerHTML = cases.length ? cases.map((story, index) => `
    <article class="document-item ${index === 0 ? "featured-document" : ""}">
      ${story.image ? `<img class="repository-list-image" src="${esc(story.image)}" alt="${esc(story.title)}">` : `<div class="document-file-icon">CASE</div>`}
      <div>
        ...
        <p>${esc(plainSummary(story.brief || story.caseIntro || "Client case content can be edited in admin."))}</p>
      </div>
      <a class="button small" href="case-study.html?case=${index}">View case</a>
    </article>
  `).join("") : `
    <article class="document-item">
      <div class="document-file-icon">CASE</div>
      <div>
        <div class="document-meta"><span>No cases</span></div>
        <h3>Add client cases in admin</h3>
        <p>Create case rows in the Stories admin section.</p>
      </div>
    </article>
  `;
}
